    ...
    try {
        ...
        if(registrationAttemptsByUser.length>=10){
            return next(new ErrorHandler(
                "You have exceeded the numner of registration attempts. Please contact support.",400
            )
        );
        }
...
export const verifyOTP=catchAsyncErrors(async(req,res,next)=>{
     ...
     try{
         ...
         }).sort({createdAt:-1});// the latest entry first
...
export const forgotPassword=catchAsyncErrors(async(req,res,next)=>{
    ...
    const user=await User.findOne({
        ...
        try{
               await sendEmail({email:user.email,subject:"Bookworm Library Management System Password recovery",
                message,
               })
               res.status(200).json({
                success:true,
    ...
    if(newPassword.length<8 ||
         newPassword.length>16 ||
          confirmNewPassword.length<8 ||
           ...
           {
              return next(new ErrorHandler("Password must be between 8 7 16 charcters",400))
           }

    if(newPassword!==confirmNewPassword){
        return next(new ErrorHandler("Confirm password and password do not match",400))
    }
